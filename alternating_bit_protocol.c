...
void A_input(struct pkt packet)
{
   ...
   if(head->packet.seqnum != packet.acknum) return;
   ...
   printf(A_PREFIX "SEND: Packet %d\n" POSTFIX, head->packet.seqnum);
   tolayer3(A, head->packet);
}
...
void A_timerinterrupt()
{
   ...
   printf(A_PREFIX "RSND: Packet %d\n" POSTFIX, A_packets_tail->next->packet.seqnum);
   tolayer3(A, A_packets_tail->next->packet);
}
